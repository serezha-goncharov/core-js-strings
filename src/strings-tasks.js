...
function removeFirstOccurrences(str, value) {
  ...
  return str
    .slice(0, index)
    .concat(str.slice(index + value.length, str.length));
}
...
function isPalindrome(str) {
  const result = [];
  const strInstance = str
    .replaceAll(' ', '')
    .replaceAll('!', '')
    .replaceAll(',', '')
    .replaceAll('?', '')
    .toLowerCase();
  for (let i = 0; i < strInstance.length; i += 1) {
    result.unshift(strInstance.charAt(i));
  }
  return result.join('') === strInstance;
}
...
function findLongestWord(sentence) {
  ...
  return array[newArray.find((element) => element.length === maxLength).index];
}
...
function extractNameFromTemplate(value) {
  return value.replaceAll('!', '').replaceAll('Hello, ', '');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < strArray.length; i += 1) {
    if (strArray[i] in rot13CipherMap) {
      strArray[i] = rot13CipherMap[strArray[i]];
    } else if (Object.values(rot13CipherMap).includes(strArray[i])) {
      strArray[i] = Object.keys(rot13CipherMap).find(
        (key) => rot13CipherMap[key] === strArray[i]
      );
    }
  }
